...
1. 使用 STM32CubeMX 配置 GPIO、RCC、SYS 等基础外设，生成可编译工程。
2. 在 HAL 框架下完成 LED 周期闪烁程序，并能够说明闪烁频率的计算过程。
3. 成功编译、下载并验证 LED 以 1 Hz 频率闪烁，记录实验现象。
4. 在实验报告中展示关键配置、核心代码、实验结果、问题排查和思考题答案。
...
| 理论要求 | 能解释 RCC 时钟配置和 `HAL_Delay()` 的工作原理 |
...
1. 可编译的完整工程目录。
2. 一份 PDF 格式实验报告。
3. CubeMX 关键配置截图、主循环代码截图、实验结果图片各至少 1 张。
4. 思考题和讨论题的书面回答。

### 报告必须回答的问题

1. 为什么本实验要先正确配置系统时钟。
2. `HAL_Delay(500)` 对应的闪烁频率是多少，如何计算。
3. 如果系统时钟不正确，LED 闪烁频率为什么会偏差。
4. 阻塞式延时在嵌入式系统中的优点和局限是什么。
...
2. 作业目标：简述 LED 闪烁实验要完成的功能和验收指标。
3. 实验原理：说明 GPIO 输出、系统时钟、`HAL_Delay()` 和闪烁频率之间的关系。
4. CubeMX 配置：至少展示 RCC、SYS、GPIO、Clock Configuration 的截图与解释。
5. 程序设计：展示主循环关键代码，并说明为什么要放在 `USER CODE` 区域。
6. 程序流程图：描述“翻转 LED -> 延时 500 ms -> 重复”的控制逻辑。
7. 实验结果：展示 LED 闪烁照片、录像截图或逻辑分析结果，并说明如何判断频率正确。
8. 思考题解答：必须完整回答本作业 8 个思考题。
9. 讨论与扩展：至少完成 3 个讨论题；若完成扩展任务，应单独给出结果与分析。
10. 问题分析与总结：记录调试过程、问题原因、解决方法和个人收获。

### 图片与流程图要求

1. 报告中至少包含 3 类图片：配置截图、代码或构建结果截图、实验现象图片。
2. 每张图片都应有图号、图题和正文引用说明。
3. 程序流程图必须单独成图，不能只用文字代替。
4. 若使用视频验证，可在报告中给出关键截图并补充说明。

### 提交规范

1. 报告文件建议命名为：`学号-姓名-作业2-LED闪烁实验.pdf`。
2. 若扩展任务未完成，应在报告中明确说明，不影响基础任务评分。
3. 若实验失败，也必须提交完整分析，不能空交。

### 最低验收标准

1. LED 能稳定闪烁，周期与设计一致。
2. 能解释 1 Hz 闪烁与 500 ms 延时的关系。
3. 能说明 `HAL_Delay()` 的使用前提和局限。
4. 报告包含真实实验结果，不得只有理论文字。
